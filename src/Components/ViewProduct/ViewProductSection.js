...
const [PronductInfo, setPronductInfo] = useState([]);
// Get product by id using useParams function 
const {id}  = useParams();
useEffect(() => {
  const fetchProductInfo = async () =>{
    const res = await axios.get(`https://golden.softgenics.in/api/ViewProduct/${id}`);
    setPronductInfo(res.data)
  }
  fetchProductInfo();
})
  const product = {
  ...
  return (
    <div>
      {PronductInfo.map((itemInfo)=>{return(<>
      <Container className="view-product">
        <span>
          <Link to="/">Home</Link> » <a onClick={() => navigate(-1)} style={{color:"black"}}> {itemInfo.category}</a> »
          {itemInfo.p_name} {itemInfo.waight}
        </span>
        
        <Row>
          <Col md={6} className="product-img-col">
            <img
              src={`https://golden.softgenics.in/uploads/${itemInfo.image}`}
              className="prd-img"
              alt=""
            />
          </Col>
          <Col md={6} className="product-disc-col mt-3">
            <h2>{itemInfo.p_name} {itemInfo.waight}</h2>
            <div className=" mb-3">
            ...
            <div className="size-type d-flex">
              ...
              <h3 className="ms-3">₹ {(itemInfo.price-itemInfo.discount)*quantity}</h3>
      ...
      <Carousel responsive={responsive} autoPlay>
        {products.map((item) => {
          if(item.category === "sunflower" || "turmeric"){
          return (
            <>
              <Card
                as={Link}
